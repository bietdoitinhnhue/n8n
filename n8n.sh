...
get_domain() {
    ...
    while [[ ! $N8N_DOMAIN =~ ^([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$ ]]; do
        read -p "Nhập domain của bạn (VD: n8n.example.com): " N8N_DOMAIN
    done
}
...
show_header() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗"
    echo "║          N8N Management Script v3 - Biệt Đội Tinh Nhuệ      ║"
    echo "╚══════════════════════════════════════════════════════════════╝${NC}"
    echo " Server IP: $(curl -s ifconfig.me) | $(date '+%Y-%m-%d %H:%M:%S') | Uptime: $(uptime -p)"
    systemctl is-active --quiet n8n && \
        echo -e "${GREEN} n8n: RUNNING $(grep 'N8N_HOST' $ENV_FILE 2>/dev/null | cut -d'=' -f2)${NC}" || \
...
install_n8n() {
    ...
    apt install -y curl gnupg2 ca-certificates lsb-release nginx ufw build-essential
